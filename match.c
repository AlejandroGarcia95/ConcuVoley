...
/* This auxiliar function, which I still don't
 * know how we should implement, might simulate
 * the time elapsed during the match. It could
 * be some sleep(s) or a huge for loop, though I
 * would consider using some other mechanism
 * which wouldn't consume my whole battery. 
 * The most important thing to note is that
 * time elapsed should be a function of the
 * difference of skills of each player. */
void simulate_time_elapsed(){

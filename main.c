...
int launch_player(unsigned int player_id) {
	...
	log_write(INFO_L, "Launching player %03d!\n", player_id);
	...
	if(!create_fifo(player_fifo_name)) {
		log_write(ERROR_L, "FIFO creation error for player %03d [errno: %d]\n", player_id, errno);
		return -1;
	}
...
int launch_court(unsigned int court_id, partners_table_t* pt) {
	log_write(INFO_L, "Launching court %03d!\n", court_id);
...
int launch_referee(partners_table_t* pt) {
	log_write(INFO_L, "Launching referee!\n");
...
int main(int argc, char **argv){
	...
	if(log_write(NONE_L, "Simulation started!") < 0){
		printf("FATAL: No log could be opened!\n");
		return -1;
	}
	log_write(INFO_L, "Main process (pid: %d) succesfully created log\n", main_pid);
	log_write(NONE_L, "Let the tournament begin!\n");
	...
	if(!pt) {
		log_write(ERROR_L, "Error creating partners table [errno: %d]\n", errno);
	}
	...
	for (i = 0; i < (TOTAL_PLAYERS + TOTAL_COURTS); i++) {
		...
		log_write(INFO_L, "Proccess pid %d finished with exit status %d\n", pid, ret);
	}

	partners_table_free_table(pt);		

	log_write(INFO_L, "Tournament ended correctly \\o/\n");
	log_close();
	return 0;
}
